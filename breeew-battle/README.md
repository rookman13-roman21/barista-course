...
- Лимит участников: `20`
- Партнёрская площадка чемпионата: `Корпоративная Академия Нефтьмагистраль и Братья Караваевы, Бизнес-центр «Россия», Волгоградский просп., 26, стр. 1` (карта: `https://yandex.ru/maps/org/biznes_tsentr_rossiya/1113761001`). Это площадка проведения чемпионата; кофе для подготовки участники по-прежнему забирают в тренинг-центре школы (см. раздел «Кофе для подготовки»).
- В публичных материалах используем формулировку «Партнёрская площадка чемпионата». Название Академии отображается обычным текстом; ссылкой на Яндекс Карты является только адрес БЦ. Не заменять этим адресом контакты школы или адрес выдачи кофе.
...
```text
https://api.barista-school.ru/api/breeew-battle.html?v=20260720-1
```
...
Sync-скрипт берёт вкладку `Тайминги` и публикует в Mini App этапы `Батлы`, `Тренировка`, `Финал` и перерывы. В тренировке три слота: менеджер вписывает участников в четыре станции каждой строки, а Mini App показывает интервалы «Выкладка / Тренировка / Уборка». Участникам видны только имена, пары и время; телефоны, email и внутренние идентификаторы не публикуются.
...
В hero-карточке «Площадка» название Академии выводится обычным текстом, а кликабельным и выделенным является только адрес БЦ. Карта берётся из `content.venue.map_url`; адрес выдачи кофе приходит отдельно из `coffee_delivery.pickup.map_url`.
...
На первом этапе участники выступают на кофе `Танзания Лунгва АВ` от The Welder Catherine. В Mini App карточка показывает пачку 250 г под фильтр и ведёт на карточку товара:

```text
https://theweldercatherine.ru/catalog/dlya_filtra/tanzaniya_lungva_av/
```
