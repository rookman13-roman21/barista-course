...
/*
 * Собирает Tilda-ready HTML из local preview. В preview остаётся только
 * безопасная демонстрация; production popup хранится отдельно в block 09.
 */
...
const mainContent = between(sourceHtml, '  <main class="mbs-la-page">\n', '\n  </main>', 'основные блоки');
...
const note = '<!-- Сгенерировано scripts/build-tilda-blocks.js из local preview. Не редактировать вручную. -->';
const assets = [
...
function writeBlock(filename, content) {
  fs.writeFileSync(path.join(blocksDir, filename), `${note}\n${content.trim()}\n`, 'utf8');
}

writeBlock('01-hero-and-for-whom.html', `${assets}\n<style>\n${sourceCss}\n</style>\n<div class="mbs-la-page">\n${mainContent.slice(0, programStart).trim()}\n</div>`);
writeBlock('02-course-content.html', `<div class="mbs-la-page">\n${mainContent.slice(programStart).trim()}\n</div>`);
