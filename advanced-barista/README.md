...
production-версией и не содержит подключений к booking API, хотя собранный
Tilda-пакет уже опубликован на `/probarista`.
...
`advanced-barista` в `schedule-online`; локальный preview по-прежнему не
создаёт записи.
...
Настоящее подключение к шести занятиям находится вне preview: оно использует
