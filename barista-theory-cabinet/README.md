...
> **Активная работа:** старый Tilda Members source поддерживается как reference; актуальная живая страница `/cabinet/theory` находится в `bitrix-tools`.
...
> Важно: текущий production-кабинет `https://baristaschool.ru/cabinet/theory` больше не обслуживается напрямую из этой папки. Актуальные lesson-фрагменты живого кабинета лежат в `../bitrix-tools/templates/gift-certificates/theory-lessons/`; эту папку использовать как исторический источник/reference.
...
### Тест «История кофе» — удалён спорный вопрос ✅

Из `barista_theory_history.html` удалён вопрос «Когда открылась первая кофейня в Санкт-Петербурге?» с вариантами `1714 / 1720 / 1812 / 1905`.

Что синхронизировано:
- подпись теста: `10 вопросов`;
- стартовый счётчик: `1 / 10`;
- нумерация следующих вопросов;
- массивы `CORRECT`, `FEEDBACK`, `WRONG_HINT`.

Для живого кабинета `/cabinet/theory?lesson=history` такая же правка внесена и задеплоена в `bitrix-tools/templates/gift-certificates/theory-lessons/history.html`.
