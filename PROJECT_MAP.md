...
| `blog/espresso-assistant/` | `/blog/espresso-assistant` | Tilda SEO-блок + hosted loader, опубликована | Помощник настройки эспрессо: сессии по зерну и дате обжарки, журнал шотов, временной гейт до вкусовых советов, одно изменение за попытку, подтверждение повторяемого рецепта и брендированный PDF-журнал. Backend отсутствует, данные остаются в `localStorage`; интерактив загружается с `https://api.barista-school.ru/api/espresso-assistant.html`. Все три PDF-действия используют общий автономный генератор бинарного A4 landscape PDF: Tilda не печатается, геометрия одинакова на desktop и мобильных устройствах. После deploy требуется контроль на реальном iPhone |
