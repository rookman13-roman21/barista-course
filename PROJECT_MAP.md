...
| `blog/espresso-assistant/` | `/blog/espresso-assistant` | Tilda SEO-блок + hosted loader, опубликована | Помощник настройки эспрессо: сессии по зерну и дате обжарки, журнал шотов, временной гейт до вкусовых советов, одно изменение за попытку, подтверждение повторяемого рецепта и брендированный PDF-журнал. Backend отсутствует, данные остаются в `localStorage`; интерактив загружается с `https://api.barista-school.ru/api/espresso-assistant.html`. PDF печатается из текущей страницы; desktop — A4 landscape, мобильный — compact portrait. Последняя мобильная визуальная приёмка после `d4adab9` ожидается |
