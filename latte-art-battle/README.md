...
- `tilda-loader.html` — короткий loader для HTML-блока Tilda.
...
- призы;
- FAQ, включая ссылку `Открыть карту` в вопросе `Где проходит чемпионат?`.
...
Для group activity есть важный нюанс: сразу после `POST /records/{company_id}` у созданной записи `record.client.id` может ещё не быть доступен. Поэтому backend делает мгновенную попытку обновить карточку клиента, а если `client_id` ещё пустой — запускает короткие фоновые повторы через `1.5с`, `5с` и `15с`. Повтор на этапе подтверждения оплаты оставлен как страховка. Данные для повторов хранятся в reservation state: имя, фамилия, телефон, email, Telegram, проект / место работы и согласия.
...
4 июня 2026 исправлена запись дополнительных полей в карточку клиента. До исправления yClients создавал запись и сохранял проект / место работы в комментарий записи, но не сохранял `telegram` и `mesto-raboti` в доп. поля клиента. Подтверждённый рабочий формат — объект `custom_fields`, затем добавлены быстрые повторы обновления карточки после создания записи, потому что `client_id` может появляться не сразу. Повторный backfill текущих клиентов события обработал 12 карточек: у всех 12 заполнено `mesto-raboti`, у 10 заполнен `telegram`, 2 Telegram не восстановлены из-за отсутствия значения в доступных комментариях yClients.
...
После оплаты и подтверждения участия backend добавляет один короткий комментарий в историю комментариев клиента yClients через поле `Новый комментарий`. В комментарий попадают только событие, email, Telegram, проект / место работы, подтверждение опыта до 3 лет и источник. Для этого сценария поле `Примечание о клиенте` не заполняется данными регистрации, а неоплаченные брони не добавляют комментарий в историю клиента.
...
Если `sold_out=true`, все CTA меняются на `Мест нет`, popup не открывается.
...
curl -sS https://api.barista-school.ru/api/latte-art-battle.html | grep -n "MBS* Latte-art"
```
...
- при sold out CTA показывает `Мест нет`
...
- Telegram у 2 записей, созданных до финального исправления 4 июня 2026, не восстановлен автоматически: значения не найдены в доступных комментариях yClients. Проект / место работы восстановлен у всех 12 текущих клиентов события.
...
- 4 июня 2026 исправлена запись доп. полей клиента: backend делает быстрые повторы после создания записи, если `client_id` ещё не доступен, и страхующий повтор на этапе подтверждения оплаты. Backfill восстановил `mesto-raboti` у всех 12 текущих клиентов события и `telegram` у 10 из 12.
