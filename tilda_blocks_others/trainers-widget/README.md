...
Новые отзывы подтягиваются из yClients через `GET /comments/{companyId}` и привязываются к сотруднику по `master_id`. После следующего запуска cron новый отзыв появляется в публичном JSON и в попапе тренера.
